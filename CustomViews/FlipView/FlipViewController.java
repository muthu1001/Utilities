...
package com.thanthi.news.FlipViewProject.flip;
...
import com.thanthi.news.FlipViewProject.utils.AphidLog;
...
public class FlipViewController extends AdapterView<Adapter> {
  ...
  private final LinkedList<View> bufferedViews = new LinkedList<>();
  ...
  public FlipViewController(Context context, AttributeSet attrs, int defStyle) {
    ...
    TypedArray
        a =
        context.getTheme().obtainStyledAttributes(attrs, com.thanthi.news.R.styleable.FlipViewController, 0, 0);

    try {
      int value = a.getInteger(com.thanthi.news.R.styleable.FlipViewController_orientation, VERTICAL);
      if (value == HORIZONTAL) {
        orientation = HORIZONTAL;
      }

      value = a.getInteger(com.thanthi.news.R.styleable.FlipViewController_animationBitmapFormat, 0);
      if (value == 1) {
